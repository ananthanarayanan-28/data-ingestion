...
-- =============================================================================
-- MATERIALIZED VIEWS  (pre-aggregated; built on top of con_master_fact)
-- =============================================================================

-- Project cost summary — Q1/Q3 type queries
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_project_cost_summary
ENGINE = SummingMergeTree()
PARTITION BY project_id
ORDER BY (project_id, epc_category, cost_code)
POPULATE AS
SELECT
    inv_project_id                          AS project_id,
    inv_li_epc_category                     AS epc_category,
    inv_li_cost_code                        AS cost_code,
    sum(inv_li_total_amount)                AS total_invoiced,
    sum(inv_budgeted_amount)                AS total_budgeted,
    sum(inv_variance_amount)                AS total_variance,
    sum(bgt_committed_amount)               AS total_committed,
    countIf(inv_duplicate_flag = 1)         AS duplicate_count,
    countIf(inv_off_contract_flag = 1)      AS off_contract_count,
    countIf(inv_price_variance_flag = 1)    AS price_outlier_count,
    count()                                 AS line_count
FROM con_master_fact FINAL
WHERE inv_project_id IS NOT NULL AND txn_type = 'invoice'
GROUP BY project_id, epc_category, cost_code;


-- Vendor spend summary — Q2/Q5 type queries
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_vendor_spend_summary
ENGINE = SummingMergeTree()
PARTITION BY project_id
ORDER BY (project_id, vendor_gstin, epc_category)
POPULATE AS
SELECT
    inv_project_id                          AS project_id,
    inv_supplier_gstin                      AS vendor_gstin,
    inv_supplier_name                       AS vendor_name,
    inv_li_epc_category                     AS epc_category,
    vnd_vendor_type                         AS vendor_type,
    sum(inv_li_total_amount)                AS total_spend,
    count(DISTINCT transaction_id)          AS invoice_count,
    countIf(inv_off_contract_flag = 1)      AS off_contract_invoices,
    countIf(inv_duplicate_flag = 1)         AS duplicate_invoices,
    avgIf(vnd_overall_rating, vnd_overall_rating IS NOT NULL) AS avg_vendor_rating
FROM con_master_fact FINAL
WHERE inv_project_id IS NOT NULL AND txn_type = 'invoice'
GROUP BY project_id, vendor_gstin, vendor_name, epc_category, vendor_type;


-- Monthly spend trend — time series analysis
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_monthly_spend_trend
ENGINE = SummingMergeTree()
PARTITION BY toYYYYMM(invoice_month)
ORDER BY (project_id, invoice_month, epc_category)
POPULATE AS
SELECT
    inv_project_id                          AS project_id,
    toStartOfMonth(inv_invoice_date)        AS invoice_month,
    inv_li_epc_category                     AS epc_category,
    sum(inv_li_total_amount)                AS total_spend,
    count(DISTINCT transaction_id)          AS invoice_count,
    count(DISTINCT inv_supplier_gstin)      AS vendor_count,
    sum(inv_variance_amount)                AS budget_variance
FROM con_master_fact FINAL
WHERE inv_project_id IS NOT NULL
  AND inv_invoice_date IS NOT NULL
  AND txn_type = 'invoice'
GROUP BY project_id, invoice_month, epc_category;


-- Project health dashboard — cross-dimension summary
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_project_health
ENGINE = ReplacingMergeTree(synced_at)
PARTITION BY project_id
ORDER BY (project_id)
POPULATE AS
SELECT
    ...
    -- Cost health
    sum(inv_li_total_amount)                    AS total_actual_cost,
    sum(inv_budgeted_amount)                    AS total_budget,
    avg(ev_cpi)                                 AS avg_cpi,
    avg(ev_spi)                                 AS avg_spi,
    -- Leakage
    countIf(inv_duplicate_flag = 1)             AS duplicate_invoices,
    countIf(inv_off_contract_flag = 1)          AS off_contract_invoices,
    countIf(inv_price_variance_flag = 1)        AS price_outlier_lines,
    sum(if(inv_duplicate_flag = 1, inv_li_total_amount, 0)) AS duplicate_amount,
    -- Safety
    sum(hse_lti_days)                           AS total_lti_days,
    countIf(hse_incident_type = 'fatality')     AS fatalities,
    -- Quality
    countIf(qua_ncr_raised = 1)                 AS ncr_count,
    sum(qua_rework_cost)                        AS total_rework_cost,
    -- Schedule
    avg(sch_actual_pct)                         AS avg_schedule_completion_pct,
    countIf(sch_delay_flag = 1)                 AS delayed_activities,
    -- Labour
    sum(lab_total_cost)                         AS total_labour_cost,
    avg(lab_productivity_index)                 AS avg_productivity_index,
    -- Equipment
    avg(eqp_utilisation_pct)                    AS avg_equipment_utilisation,
    sum(eqp_total_running_cost)                 AS total_equipment_cost
FROM con_master_fact FINAL
WHERE inv_project_id IS NOT NULL
GROUP BY project_id;